...
def loaddata(fileName, label = False):
    ...
    header = ['Dur','Proto', 'State','TotPkts','TotBytes']
...
def extract_data(fileName, SrcAddr, DstAddr, top = None, key = None):
    protoDict = {'arp': 5, 'unas': 13, 'udp': 1, 'rtcp': 7, 'pim': 3, 'udt': 11, 'esp': 12, 'tcp' : 0, 'rarp': 14, 'ipv6-icmp': 9, 'rtp': 2, 'ipv6': 10, 'ipx/spx': 6, 'icmp': 4, 'igmp' : 8}
    ...
    for index, row in df.iterrows(): 
        ...
        if Sip == SrcAddr and Dip == DstAddr:
            ...
            if value == "empty":
                ls += int(totB)
                data[f"{str(Sport)} -> {str(Dport)}"] = {"packet": (int(totP)), "byte": (int(totB)), "dur": str(Decimal(dur))}
                ...
                ls += int(totB)
                cur_packets = data[f"{str(Sport)} -> {str(Dport)}"].get("packet")
                cur_bytes = data[f"{str(Sport)} -> {str(Dport)}"].get("byte")
                cur_dur = Decimal(data[f"{str(Sport)} -> {str(Dport)}"].get("dur"))
